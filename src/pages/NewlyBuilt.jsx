...
class NewlyBuilt extends Component  {
  ...
  findAllProperty(currentPage) {
      currentPage -=1;
    fetch("http://localhost:2020/property/status/Newly Built?page=" +this.state.currentPage+"&size="+this.state.itemsPerPage)
        .then((res) => res.json())
        .then((json) => {
            this.setState({
                items: json.content,
