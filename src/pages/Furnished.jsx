...
class Furnished extends Component  {
  ...
  componentDidMount() {
    ...
    this.firstPageMove(this.state.currentPage)
    this.prevPageMove(this.state.currentPage)
  }
  ...
  findAllProperty(currentPage) {
      currentPage -=1;
    fetch("http://localhost:2020/property/status/Furnished?page=" +this.state.currentPage+"&size="+this.state.itemsPerPage)
        .then((res) => res.json())
        .then((json) => {
            this.setState({
                items: json.content,
  ...
  firstPage = () => {
    ...
    if(this.state.currentPage > firstPage){
     this.firstPageMove(firstPage);
    }
  };

  prevPage = () => {
    let prevPage = 1;
    let currentPage = this.state.currentPage - prevPage
    if(this.state.currentPage > prevPage){
    
      this.prevPageMove(currentPage- prevPage);
    }
  ...
  if(this.state.currentPage < condition ){
      this.setState({
        currentPage: this.findAllProperty(condition)
      });
