...
class Serviced extends Component  {
  ...
  findAllProperty(currentPage) {
      currentPage -=1;
    fetch("http://localhost:2020/property/status/Serviced?page=" +this.state.currentPage+"&size="+this.state.itemsPerPage)
        .then((res) => res.json())
        .then((json) => {
            this.setState({
                items: json.content,
  ...
  firstPage = () => {
    ...
    if(this.state.currentPage > firstPage){
      this.findAllProperty(firstPage);
    }
  };
  ...
  prevPage = () => {
    ...
    if(this.state.currentPage > prevPage){
      this.findAllProperty(this.state.currentPage - prevPage);
    }
  };
  ...
  lastPage = () => {
    ...
    if(this.state.currentPage < condition ){
      this.findAllProperty(condition)
    }
  ...
  nextPage = () => {
    if(this.state.currentPage < Math.ceil(this.state.totalElements / this.state.itemsPerPage) ){
      this.findAllProperty(this.state.currentPage + 1)
    }
