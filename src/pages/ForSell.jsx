...
class ForSell extends Component  {
  ...
  findAllProperty(currentPage) {
      currentPage -=1;
    fetch("http://localhost:2020/property/purpose/For-Sell?page=" +this.state.currentPage+"&size="+this.state.itemsPerPage)
        .then((res) => res.json())
        .then((json) => {
            this.setState({
                items: json.content,
  ...
  firstPage = (event) => {
    ...
    if(this.state.currentPage > firstPage){
      this.setState({
        currentPage : this.findAllProperty(firstPage)
      });
    }
  };
  ...
  prevPage = () => {
    ...
    if(this.state.currentPage > prevPage){
      this.setState({
        currentPage: this.findAllProperty(this.currentPage - prevPage)
      });
    }
  };
  ...
  nextPage = () => {
    if(this.state.currentPage < Math.ceil(this.state.totalElements / this.state.itemsPerPage) ){
      this.setState({
        currentPage: this.findAllProperty(this.state.currentPage + 1)
      });
    }
